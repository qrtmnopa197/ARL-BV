...
# all_data <- lapply(csvs_to_analyze,wrangle_psychopy_data) #reformats each CSV, turning it into a long dataset usable for analysis, and adds all variables of interest that can be created from the raw data alone.
...
# tl_hists <- c("dec_probe_rt","dec_rt","feed_probe_rt","valence_dec","arousal_dec","valence_feed","arousal_feed") #trial level variables to plot
# plot_trial_level_vars(trial_level_data,tl_hists,path_to_project_directory) #create and save plot grids
# sl_hists <- c("consecutive_late_choices","att_checks_passed","consecutive_auto_process","total_auto_process",
#               "answers_correct","answers_incorrect","noprobe_pt_choices","probe_pt_choices",
#               "earnings_before_ec","feed_attention_check_numeric","percent_left","percent_correct","late_percent","trials_completed",
#               "av_outcome_diff_p","probe_skipped_percent","affect_attention_check_numeric","sd_valence_dec","sd_arousal_dec",
#               "sd_valence_feed","sd_arousal_feed","total_experiment_time") #subject-level variables to plot
