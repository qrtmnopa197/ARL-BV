...
```{r}
# Get necessary data
trials_bqba_sim <- trials %>% select(sub_index,block,overall_trial_nl,chosen_frac,unchosen_frac,fA_ix,fB_ix,rat_number)
...
# Estimate effects of Q and A on choice for each simulated dataset
sim_bQ <- c()
sim_bA <- c()
# Loop through every posterior draw
for(i in 1:dim(sim_rat)[1]){
  for(c in 1:dim(sim_rat)[2]){
    # Add simulated choices and ratings to the data
    sim_trials <- trials_bqba_sim %>% 
                    mutate(choice=as.vector(sim_choice[i,c,])) %>%
                    left_join(data.frame(valrat_z = as.vector(sim_rat[i,c,]),rat_number = 1:dim(sim_rat)[3]),
                              by="rat_number") 
    # Get estimated effects of Q and A on choice
    est_eff <- est_bq_ba(sim_trials)
    sim_bQ <- c(sim_bQ,est_eff$bQ)
    sim_bA <- c(sim_bA,est_eff$bA)
  }
}
```
