...
// No choice-autocorrelation component
...
parameters{
  ...
  vector[n_s] dcy_z;
  //Learning rates for Q-values and A-values
...
transformed parameters {
  ...
  {//anonymous_scope_start
    ...
    for (s in 1:n_s) {
      for (t in 1:n_t){
        if(t == 1){
          // for the first trial of each subject, set all Q/A values to 0
          Q[s,t] = rep_vector(0,n_f); 
          A[s,t] = rep_vector(0,n_f); 
        }
        
        softmax_arg = Q[s,t,{fA[s,t],fB[s,t]}] + A[s,t,{fA[s,t],fB[s,t]}]; //set argument for softmax decision function
