// Same as final_ls.stan, except this outputs the the term (bq(Q_a-Q_b) + bq(A_a-A_b)) for each trial
  ...
  {//anonymous_scope_start
    ...
    for (s in 1:n_s) {
      for (t in 1:n_t){
        ...
        q_a_term[(s-1)*n_t+t] = rew_fr_sens[s]*(Q_fr[s,t,fA[s,t]] - Q_fr[s,t,fB[s,t]]) + aff_fr_sens[s]*(A_fr[s,t,fA[s,t]] - A_fr[s,t,fB[s,t]]);
