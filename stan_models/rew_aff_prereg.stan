data {
  ...
  vector[n_rat] prev_rat; //the previous valence rating (used for autoregressive term)
}
...
transformed parameters {
  ...
  {//anonymous_scope_start
    ...
    vector[n_s] dcy_fr = inv_logit(dcy_fr_mu + dcy_fr_sigma*dcy_f_z);
    ...
    for (s in 1:n_s) {
      for (t in 1:n_t){
        ...
        if(fres[s,t] == 1){
          ...
          curr_pred = B_0[s] + B_rew_fr[s]*rew[s,t] + B_bv_fr[s]*bv[s,t] + B_auto[s]*prev_rating[s,t];
        }else if (fres[s,t] == 0){
          //if not...
          curr_pred = B_0[s] + B_rew_nf[s]*rew[s,t] + B_auto[s]*prev_rating[s,t];
        }
        
        if(rat_num[s,t] != 0){
          //if the participant made a valence rating... 
          rat_pred[rat_num[s,t]] = curr_pred; //add to the vector of predicted ratings
          affect = rat[s,t]; //affect is equal to the rating made
        } else{
