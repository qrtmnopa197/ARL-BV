...
parameters{
  ...
  real rewnf_b_mu;
  real<lower=0> rewnf_b_sigma;
  vector[n_s] rewnf_b_z;
  ...
  real bvf_b_mu;
  real<lower=0> bvf_b_sigma;
  vector[n_s] bvf_b_z;
  
  //choice sensitivity to affect when fractal is/isn't presented
  real asens_f_mu;
  real<lower=0> asens_f_sigma;
  vector[n_s] asens_f_z;
  
  real asens_nf_mu;
  real<lower=0> asens_nf_sigma;
  vector[n_s] asens_nf_z;
  ...
  //decay rates
  real dcy_q_mu;
  real<lower=0> dcy_q_sigma;
  vector[n_s] dcy_q_z;
  
  real dcy_a_mu;
  real<lower=0> dcy_a_sigma;
  vector[n_s] dcy_a_z;
  
  real dcy_b_mu;
  real<lower=0> dcy_b_sigma;
  vector[n_s] dcy_b_z;
  
  real dcy_qnf_mu;
  real<lower=0> dcy_qnf_sigma;
  vector[n_s] dcy_qnf_z;
  
  real dcy_anf_mu;
  real<lower=0> dcy_anf_sigma;
  vector[n_s] dcy_anf_z;
...
transformed parameters {
  ...
  {//anonymous_scope_start
    ...
    vector[n_s] rewf_b = rewf_b_mu + rewf_b_sigma*rewf_b_z;
    vector[n_s] rewnf_sens = rewnf_sens_mu + rewnf_sens_sigma*rewnf_sens_z;
    vector[n_s] rewnf_b = rewnf_b_mu + rewnf_b_sigma*rewnf_b_z;
    vector[n_s] bvf_sens = bvf_sens_mu + bvf_sens_sigma*bvf_sens_z;
    vector[n_s] bvf_b = bvf_b_mu + bvf_b_sigma*bvf_b_z;
    vector[n_s] asens_f = asens_f_mu + asens_f_sigma*asens_f_z;
    vector[n_s] asens_nf = asens_nf_mu + asens_nf_sigma*asens_nf_z;
    vector[n_s] csens = csens_mu + csens_sigma*csens_z;
    
    //NCP for normal priors with sigmoid transform
    vector[n_s] dcy_q = inv_logit(dcy_q_mu + dcy_q_sigma*dcy_q_z);
    vector[n_s] dcy_a = inv_logit(dcy_a_mu + dcy_a_sigma*dcy_a_z);
    vector[n_s] dcy_b = inv_logit(dcy_b_mu + dcy_b_sigma*dcy_b_z);
    vector[n_s] dcy_qnf = inv_logit(dcy_qnf_mu + dcy_qnf_sigma*dcy_qnf_z);
    vector[n_s] dcy_anf = inv_logit(dcy_anf_mu + dcy_anf_sigma*dcy_anf_z);
    ...
    for (s in 1:n_s) {
      for (t in 1:n_t){
        ...
        softmax_arg = rewf_sens[s]*Q[s,t,{fA[s,t],fB[s,t]}] + asens_f[s]*A[s,t,{fA[s,t],fB[s,t]}] + bvf_sens[s]*B[s,t,{fA[s,t],fB[s,t]}] + 
                      rewnf_sens[s]*Q_nf[s,t,{fA[s,t],fB[s,t]}] + asens_nf[s]*A_nf[s,t,{fA[s,t],fB[s,t]}] + csens[s]*C[s,t,{fA[s,t],fB[s,t]}] + [lsbias[s],0]';
        ...
        if(rat_num[s,t] != 0){
          if(fres[s,t] == 1){
            rat_pred[rat_num[s,t]] = incp[s] + rewf_b[s]*rew[s,t] + bvf_b[s]*bv[s,t];
          }else if (fres[s,t] == 0){
            rat_pred[rat_num[s,t]] = incp[s] + rewnf_b[s]*rew[s,t];
          }
        ...
        if(t < n_t){
          ...
          Q[s,t+1] = (1-dcy_q[s])*Q[s,t];
          A[s,t+1] = (1-dcy_a[s])*A[s,t];
          B[s,t+1] = (1-dcy_b[s])*B[s,t];
          Q_nf[s,t+1] = (1-dcy_qnf[s])*Q_nf[s,t];
          A_nf[s,t+1] = (1-dcy_anf[s])*A_nf[s,t];
          if(choice[s,t] == 1){
            // if fA was chosen...
            ...
            if(fres[s,t] == 1){
              ...
              A[s,t+1,fA[s,t]] = A[s,t,fA[s,t]] + lrn_a[s]*(resid - A[s,t,fA[s,t]]);
              ...
              A_nf[s,t+1,fA[s,t]] = A_nf[s,t,fA[s,t]] + lrn_anf[s]*(resid - A_nf[s,t,fA[s,t]]);
            }
            ...
            if(fres[s,t] == 1){
              ...
              A[s,t+1,fB[s,t]] = A[s,t,fB[s,t]] + lrn_a[s]*(resid - A[s,t,fB[s,t]]);
              ...
              A_nf[s,t+1,fB[s,t]] = A_nf[s,t,fB[s,t]] + lrn_anf[s]*(resid - A_nf[s,t,fB[s,t]]);
            }
          }
          //update C
...
model{
  ...
  //effects on affect
  rewf_b_mu ~ normal(0,40); 
  rewf_b_sigma ~ normal(0,60); 
  rewnf_b_mu ~ normal(0,.7); 
  rewnf_b_sigma ~ normal(0,1); 
  bvf_b_mu ~ normal(0,.7);
  bvf_b_sigma ~ normal(0,1);
  //effects on choice
  ...
  asens_f_mu ~ normal(0,6);
  asens_f_sigma ~ normal(0,8);
  asens_nf_mu ~ normal(0,6);
  asens_nf_sigma ~ normal(0,8);
  csens_mu ~ normal(0,5);
  csens_sigma ~ normal(0,10);
  
  
  //learning and decay rates
  dcy_q_mu ~ normal(-.05,1.7);
  dcy_q_sigma ~ normal(0,4);
  dcy_a_mu ~ normal(-.05,1.7);
  dcy_a_sigma ~ normal(0,4);
  dcy_b_mu ~ normal(-.05,1.7);
  dcy_b_sigma ~ normal(0,4);
  dcy_qnf_mu ~ normal(-.05,1.7);
  dcy_qnf_sigma ~ normal(0,4);
  dcy_anf_mu ~ normal(-.05,1.7);
  dcy_anf_sigma ~ normal(0,4);
  ...
  rewnf_b_z ~ std_normal();
  rewnf_sens_z ~ std_normal();
  bvf_b_z ~ std_normal();
  bvf_sens_z ~ std_normal();
  asens_f_z ~ std_normal();
  asens_nf_z ~ std_normal();
  csens_z ~ std_normal();
  dcy_q_z ~ std_normal();
  dcy_a_z ~ std_normal();
  dcy_b_z ~ std_normal();
  dcy_qnf_z ~ std_normal();
  dcy_anf_z ~ std_normal();
  lrn_q_z ~ std_normal();
  lrn_a_z ~ std_normal();
  lrn_b_z ~ std_normal();
