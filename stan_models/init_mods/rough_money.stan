  ...
  {//anonymous_scope_start
    ...
    for (s in 1:n_s) {
      for (t in 1:n_t){
        ...
        softmax_arg = [beta[s]*Q[s,t,fA[s,t]] + phi[s]*C[s,t,fA[s,t]],
                       beta[s]*Q[s,t,fB[s,t]] + phi[s]*C[s,t,fB[s,t]]]'; //set softmax arg outside of the log_lik calculation for efficiency
