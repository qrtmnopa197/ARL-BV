// Same as init_mo_bv, but renamed so that the files would save to the appropriate place
// ~DP
data {
...
parameters {
  ...
  real blearn_mu; // mean of group-level learning rate distribution
  real<lower=0> blearn_sigma; // sd of group-level learning rate distribution
  vector[n_s] blearn_z; // z-score of subject-level alpha value (relative to group distribution)
 
  real bsens_mu; // mean of group-level inverse temperature distribution
  real<lower=0> bsens_sigma; // sd of group-level inverse temperature distribution
  vector[n_s] bsens_z; // z-score of subject-level beta value (relative to group distribution)
...
transformed parameters {
  ...
  {//anonymous_scope_start
    ...
    vector[n_s] bsens = bsens_mu + bsens_sigma*bsens_z;  // get subject-level beta values using n.c.p.
    ...
    vector[n_s] blearn = inv_logit(blearn_mu + blearn_sigma*blearn_z); //diddo alpha. Also, use inv_logit to get alpha between 0 and 1
    ...
    for (s in 1:n_s) {
      for (t in 1:n_t){
        if(t == 1){
          ...
          B[s,t] = rep_vector(0,n_f); // for the first trial of each subject, set all Q-values to 0
          C[s,t] = rep_vector(0,n_f);
        }
        
        softmax_arg = [beta[s]*Q[s,t,fA[s,t]] + beta_nfr[s]*Q_nfr[s,t,fA[s,t]] + bsens[s]*B[s,t,fA[s,t]] + phi[s]*C[s,t,fA[s,t]],
                       beta[s]*Q[s,t,fB[s,t]] + beta_nfr[s]*Q_nfr[s,t,fB[s,t]] + bsens[s]*B[s,t,fB[s,t]] + phi[s]*C[s,t,fB[s,t]]]'; //set softmax arg outside of the log_lik calculation for efficiency
        ...
        if(t < n_t){
          ...
          if(choice[s,t] == 1){
            ...
            if(fres[s,t] == 1){
              ...
              B[s,t+1,fA[s,t]] = B[s,t,fA[s,t]] + blearn[s]*(bv[s,t]-B[s,t,fA[s,t]]);
            } else if(fres[s,t] == 0){
            ...
            if(fres[s,t] == 1){
              ...
              B[s,t+1,fB[s,t]] = B[s,t,fB[s,t]] + blearn[s]*(bv[s,t]-B[s,t,fB[s,t]]);
            } else if(fres[s,t] == 0){
              //otherwise update Q_nfr
...
model {
  ...
  blearn_mu ~ normal(-.05,1.7); //This yields a nearly uniform 0-1 prior when logit transformed
  blearn_sigma ~ normal(0,4); //allow for large inter-subject heterogeneity without enforcing marginal subject-level priors that are excessively horeshoe-shaped
  ...
  bsens_mu ~ normal(1,5); // Weakly informative prior
  bsens_sigma ~ normal(0,5); //Setting the prior on the SD with similar logic
