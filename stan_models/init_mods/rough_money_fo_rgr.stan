...
parameters {
  ...
  real fsens_mu; // mean of group-level inverse temperature distribution
  real<lower=0> fsens_sigma; // sd of group-level inverse temperature distribution
...
transformed parameters {
  ...
  vector[n_s] beta = beta_mu + beta_sigma*beta_z;
  vector[n_s] fsens = fsens_mu + fsens_sigma*fsens_z;
...
model {
  ...
  fsens_mu ~ normal(2,10);
  fsens_sigma ~ normal(0,10);
...
generated quantities {
  ...
  real ratio_r;
  {
    vector[n_s] aff_ratio;
    vector[n_s] sens_ratio;
    vector[n_s] aff_ratio_stand;
    vector[n_s] sens_ratio_stand;
    vector[n_s] fob_stand = (fo_b - mean(fo_b))/sd(fo_b);
    vector[n_s] fsens_stand = (fsens - mean(fsens))/sd(fsens);
    vector[n_s] rewb_stand = (reward_b - mean(reward_b))/sd(reward_b);
    vector[n_s] beta_stand = (beta - mean(beta))/sd(beta);
    
    
    for(s in 1:n_s){
      aff_ratio[s] = fo_b[s]/reward_b[s];
      sens_ratio[s] = fsens[s]/beta[s];
    }
    aff_ratio_stand = (aff_ratio - mean(aff_ratio))/sd(aff_ratio);
    sens_ratio_stand = (sens_ratio - mean(sens_ratio))/sd(sens_ratio);
  
    //get correlation
    fsens_fob_r = (dot_product(fsens_stand,fob_stand))/(n_s-1);
    beta_rewb_r = (dot_product(beta_stand,rewb_stand))/(n_s-1);
    ratio_r = (dot_product(aff_ratio_stand,sens_ratio_stand))/(n_s-1);
  }
}
