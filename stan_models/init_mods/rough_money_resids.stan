// This is a rough first-pass at an RL model that includes effects of both reward and affect residuals,
...
data {
  ...
  array[n_s] vector[n_t] aff_resid; // 1d array of vectors - one for each subject - that contains the valence rating z score on each trial
...
transformed parameters {
  ...
  {//anonymous_scope_start
    ...
    vector[n_s] asens = asens_mu + asens_sigma*asens_z;
    ...
    vector[n_s] alearn = inv_logit(alearn_mu + alearn_sigma*alearn_z);
    ...
    for (s in 1:n_s) {
      for (t in 1:n_t){
        ...
        softmax_arg = [beta[s]*Q[s,t,fA[s,t]] + phi[s]*C[s,t,fA[s,t]] + asens[s]*A[s,t,fA[s,t]],
                       beta[s]*Q[s,t,fB[s,t]] + phi[s]*C[s,t,fB[s,t]] + asens[s]*A[s,t,fB[s,t]]]'; //set softmax arg outside of the log_lik calculation for efficiency
        ...
        if(t < n_t){
          Q[s,t+1] = (1-alpha[s])*Q[s,t]; // Decay Q values toward their initial values - 0. The only values
                                          ...
                                          // updated below.
          A[s,t+1] = (1-alearn[s])*A[s,t]; // Decay A values toward their initial values - 0

          if(choice[s,t] == 1){
            // if fA was chosen...
            // update the Q and A values for fA
            Q[s,t+1,fA[s,t]] = Q[s,t,fA[s,t]] + alpha[s]*(reward[s,t]-Q[s,t,fA[s,t]]);
            A[s,t+1,fA[s,t]] = A[s,t,fA[s,t]] + alearn[s]*(aff_resid[s,t]-A[s,t,fA[s,t]]);
            ...
            A[s,t+1,fB[s,t]] = A[s,t,fB[s,t]] + alearn[s]*(aff_resid[s,t]-A[s,t,fB[s,t]]);
            // C value update
            choice_a = 0;
            choice_b = 1;
          }
...
model {
  ...
  alearn_mu ~ normal(-.05,1.7);
  alearn_sigma ~ normal(0,4);
  ...
  asens_mu ~ normal(2,10);
  asens_sigma ~ normal(0,10);
